# Week 1 — Walking Skeleton in Production ✔ (closed 2026-08-23)

> Archived record. Current work is `tasks/todo.md`.
...
- [x] Email+password session login gating `/admin` — one seeded admin user, roles come week 2. **scrypt, not argon2**: `node:crypto` has it built in, so the container needs no native build toolchain; dona-v2 also shipped scrypt despite its roadmap saying otherwise. ROADMAP week 2 amended with the reason; argon2id revisited in week 6
- [x] Five things v2's auth got wrong, fixed and written into `SPEC-staff.md`: cost parameters stored in the record so scrypt's cost can be raised later · unknown email and wrong password indistinguishable in code, message and timing · `sha256(token)` stored instead of the token · clock injected, no `Date.now()` · a 5-in-15-minutes failed-attempt throttle that refuses a correct password too
- [x] Seeding from Secret Manager at boot: idempotent, a no-op when unset, loud when half-configured or when the password is too short. `bootstrap.sh` creates both secrets per environment so staging and prod cannot drift
- [x] Week 1 checkpoint ritual: week demo run (prod on a phone behind a login · CI blocking a broken PR, evidence from 3.2 · rollback rehearsed again on the real release); ROADMAP week 1 fully ticked; week 2's todo written
- [x] Tag `v0.1.0` → prod

**Done when:** prod URL shows the login page; wrong password fails; ROADMAP week 1 fully ticked. ✔
**Verify:** `v0.1.0` live on prod. · **Verified 2026-08-23: [release run](https://github.com/RandomWilder/dona-v3/actions/runs/32636887233) green in 1m46s → `dona-prod-00003-8jc`, boot line `staff seed: created`. https://dona-prod-ydabrrmura-zf.a.run.app/admin redirects to the login page, a wrong password is refused with no cookie issued, and the session cookie carries `HttpOnly; SameSite=Lax; Secure` behind Cloud Run's TLS termination. Owner logged in on a phone against staging and prod. Rollback rehearsed on the real release: 11s back, 8s forward. Full record: `tasks/evidence/5.2-login-live.md`** · Size: S
