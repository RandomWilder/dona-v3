# Week 2 — Identity, portfolio, occupancy, roles  ✔ CLOSED 2026-08-25
...
- [x] **The viewer account, which had never existed.** Everything behind the demo was tested at 9.1 except the second actor: the seeder creates but never updates, there is no operator-management screen, and `administer` has no command — so a second operator was a manual `INSERT`. `seedStaffViewer` is that account arriving by the one route that leaves a diff. **Two entry points over one internal function, not a role parameter with a default:** 9.1 rejected a `STAFF_SEED_ROLE` because a value that could say `viewer` deploys a system nobody can administer, and a *defaulted* argument fails **open** to admin when a caller forgets. Each door names its own role as a literal
- [x] The two differ in what a missing configuration *means*, which is why they are separate at the contract seam rather than one call with a flag: **a missing admin throws** (a system with no way in), **a missing viewer is a silent no-op** (one less demo account), and half of either pair throws with the message naming the pair that is actually half-set. Local dev and any environment that has not set the secrets boots exactly as before
- [x] `bootstrap.sh` creates both viewer secrets per environment so staging and prod cannot drift, idempotently — an existing secret is left untouched, which is what let Asaf's hand-created staging values survive a re-run. `deploy.yml` and `release.yml` both pass them. **The ordering this created was real:** `--set-secrets` naming a secret that does not exist fails the deploy, so `bootstrap.sh prod` had to run before the tag
- [x] Run the week demo; tick ROADMAP week 2; write week 3's todo
- [x] Tag `v0.2.0` → prod

**Done when:** viewer vs admin demonstrated on prod; ROADMAP week 2 fully ticked.
**Verify:** `v0.2.0` live on prod. · Size: S → M, given the account had to be built first

**Verified 2026-08-25:** 283 tests pass, 0 skipped (279 → 283) — `npm run typecheck` · `npm run lint` · `REQUIRE_POSTGRES=1 npm test` · `npm run evals` → 3/3. **The four existing seed tests are unedited**, which is the proof the refactor changed nothing, and **the bar was checked by breaking it**: seed the viewer as `admin` instead and two tests fail, including the one that reads the role back out of a real login. Staging's boot line — the machine-checkable half — reads `staff seed: already exists · viewer seed: created`. Record: `tasks/evidence/day-10-week-2-close.md`
