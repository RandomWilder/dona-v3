...
- **Third parties that see tenant text are named here, not discovered later.** As of slice 12.2 lease clause text is sent to OpenAI to be embedded — a real contract's contents leaving our infrastructure. **Since 14.1b what is embedded is narrower than what is stored:** a chunk with no clause reference is never sent, which excludes the front page and therefore both parties' names, ID numbers, phone, email and addresses. The same rule 13.1 applies to extraction, arrived at from the other direction — and it was taken here for an accuracy reason and a privacy one at once, because the uncitable chunk was also the one winning questions it had nothing to do with. The consent basis is in the lease itself, which contemplates the landlord passing tenant data to third parties providing IT services (`docs/reference/lease-template-donadom.md`), and the same note's warning applies: worth knowing, and worth not exceeding. Any further processor gets a line here before it gets a call.
